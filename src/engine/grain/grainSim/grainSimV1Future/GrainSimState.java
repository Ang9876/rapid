...
public class GrainSimState extends GrainState {
    ...
    public GrainSimState(HashSet<Thread> tSet, HashMap<String, Thread> tMap) {
        threadSet = tSet;
        threadMap = tMap;
    ...
    public boolean update(GrainEvent e) {
        ...
        HashSet<Thread> futureThreads = new HashSet<>();
        String locs = String.valueOf(e.getLocId());
        // System.out.println(locs);
        for(int i = 0; i < locs.length(); i += 2) {
            String t = locs.substring(i, i + 2);
            futureThreads.add(threadMap.get("T" + t));
        }
        ...
        while(iter.hasNext()){
            ...
            if(state.currentGrain.threads.isEmpty()) {
            ...
            //     continue;
            // }
            for(Thread t: futureThreads) {
                if(state.afterSetThreads.contains(t) != state.inFrontier) {
                    iter.remove();
                    continue;
                }
            }
            ...
            // Many threads optimization (incomplete) 
            if(state.currentGrain.threads.size() > 2) {
                iter.remove();
                continue;
            }
            ...
            if(!witnessE1 || state.containsE2 || (e.getType().isWrite() && !e.isE1 && state.currentGrain.size != 1)) {
                continue;
            }
...
class NondetState {
    ...
    public NondetState(NondetState state) {
        ...
        afterSetThreads = new HashSet<>();
        afterSetThreads.addAll(state.afterSetThreads);
