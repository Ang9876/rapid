package engine.grain.grainSim.grainSimV1ContractMini;
    ...
    public boolean update(GrainEvent e) {
        ...
        while(iter.hasNext()){
            ...
            if(e.getType().isRead() && state.threads.isEmpty() && 
                (state.afterSetWtVars.contains(e.getVariable()) || state.afterSetThreads.contains(e.getThread()))) {
                singleRead = true;
            }
            ...
            if(!witnessE1 || state.containsE2 || (e.getType().isWrite() && !e.isE1 && state.size != 1)) {
